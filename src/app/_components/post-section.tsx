...
export default function PostSection({ data, onClickEdit, onClick, demo }: { data: IPostSection, onClickEdit?: () => void, demo?: boolean, onClick?: () => void }) {
    ...
    return (
        <div onClick={onClick} className={"dark:text-white " + (onClick ? "cursor-pointer" : "") + (demo ? "max-h-44 flex flex-row" : "flex flex-col")}>
            ...
            <Box className="flex items-center justify-center">
                {data.media && <img src={data.media} alt="" className={"text-center mt-7 " + (demo ? "max-w-32" : "w-full")} />}
            </Box>
        </div>
    )
}