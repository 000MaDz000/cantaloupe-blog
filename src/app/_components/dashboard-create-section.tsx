...
export default function CreateSection({ onSave }: { onSave: (title: string, body: string) => void }) {
        ...
        <Box className="flex flex-col gap-4">
            ...
            <Box className="flex flex-col gap-10 dark:[&_*]:placeholder:text-gray-300 dark:[&_*]:caret-white dark:[&_*]:text-gray-200">
                <Box className="flex flex-col gap-3 dark:border-2 dark:border-sky-900 rounded-md">
                    ...
                    <TextField fullWidth inputRef={title} onChange={() => titleErr ? setTitleErr("") : ""} name="section-title" variant="outlined" color={titleErr ? "error" : undefined} placeholder={t("section title placeholder")} />
                </Box>
                <TextareaAutosize ref={body} name="section-title" color="secondary" placeholder={t("section body placeholder")} className="min-h-56 outline-none p-5 border dark:bg-transparent dark:border-2 dark:border-sky-900 rounded-md" />
